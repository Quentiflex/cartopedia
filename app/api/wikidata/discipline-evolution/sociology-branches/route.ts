...
type Row = {
  ...
  endYear: null;
  description?: string;
  wikiSummary?: { extract: string; articleUrl: string; thumbnail?: string } | null;
};

const SOCIOLOGY_IRI = "http://www.wikidata.org/entity/Q21201";
const BRANCH_OF_SOCIOLOGY_IRI = "http://www.wikidata.org/entity/Q105760881";

async function enrich(rows: Array<{ iri: string; label: string; description?: string; articleUrl?: string }>): Promise<Row[]> {
  return Promise.all(
    rows.map(async (r, i) => {
      const wikiSummary =
        i < 12 && r.articleUrl
...
SELECT ?item ?label ?description ?articleUrl WHERE {
  ?item wdt:P31 <${BRANCH_OF_SOCIOLOGY_IRI}> .
...
LIMIT ${limit}`.trim();
    ...
    return NextResponse.json({
      ...
      schoolType: { iri: BRANCH_OF_SOCIOLOGY_IRI, label: "branch of sociology" },
    ...
    const msg = err instanceof Error ? err.message : "Failed to fetch sociology branches";
